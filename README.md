...
VinylDB includes an MCP (Model Context Protocol) server to facilitate agentic interactions with the database and external resources.

### Running the Server

```bash
.\server\venv\Scripts\python -m server.mcp_server
```

### Tools

-   **`add_vinyl`**: Adds a new vinyl record to the database.
    -   Inputs: `title`, `artist`, `year`, `cat_no`, `genre`
-   **`list_vinyls`**: Lists all vinyl records currently in the collection.
-   **`search_online_metadata`**: Searches MusicBrainz for vinyl release metadata.
    -   Inputs: `query` (e.g., "Abbey Road Beatles")
-   **`search_web`**: Searches DuckDuckGo for general information about a vinyl.
    -   Inputs: `query`
